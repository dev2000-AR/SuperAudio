...
function Library() {
  ...
  useEffect(() => {
    const fetchAlbums = async () => {
      try {
        // Verificar que la cookie "user" existe antes de intentar acceder a ella
        const userCookie = document.cookie.split('; ').find(row => row.startsWith('user='));
        if (userCookie) {
          const token = JSON.parse(userCookie.split('=')[1]).token;
      ...
      <div className="w-full min-h-[1000px] px-6 pt-24 mobile:px-4 ">
        ...
        >
          <Link href="/collection/liked">
            <div
              className="flex flex-col p-3.5 cursor-pointer p-3.5 bg-gradient-to-t from-[#2c2a2a4a] to-[#2c2a2ac7] hover:bg-[#4340409d]
          ...
          {albums.map((album: any) => (
            <Link key={album.idcre} href={`/album/${album.idcre}`}>
              <div
                className="cursor-pointer p-3.5 bg-gradient-to-t from-[#2c2a2a4a] to-[#2c2a2ac7] hover:bg-[#4340409d]
           tablet:hover:bg-transparent mobile:hover:bg-transparent 
           rounded-md h-full mini-laptop:p-3 tablet:p-0 tablet:from-transparent tablet:to-transparent
           ...
           "
              >
                ...
                >
                  <Image
                    src={album.cover_image || "/default_cover_image.jpg"}
                    width="300"
                    height="300"
                    objectFit="cover"
                    layout="responsive"
                ...
                <div className="py-3">
                  <p className="">{album.titulo}</p>
                  <p className="text-gray-300 text-sm mt-1">
                    {album.cdtracks} Tracks
                  </p>
                </div>
              </div>
            </Link>
