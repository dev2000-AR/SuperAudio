...
function Album({
  ...
  return (
    <AppLayout title={album.name} color={album.color} onScroll={onScroll}>
      <NavBar
      ...
      <div
        style={{ backgroundColor: shadeColor(album.color, -30) }}
        ...
        <div
          style={{
            backgroundColor: album.color
              ? shadeColor(album.color, -30)
              : "#121212",
          }}
          className="rounded mr-6 tablet:mr-0 w-[230px] min-w-[230px] h-[230px] mobile:mr-0 relative"
        >
          <CustomImage src={album.cover_image + "&auto=format&fit=crop&w=400&q=50&h=400"} />
        </div>
        <div>
      ...
      <div
        ...
        style={{
          background: `linear-gradient(180deg, ${
            album.color ? shadeColor(album.color, -60) : "#121212"
...
export async function getServerSideProps(context: any) {
  ...
  try {
    ...
    const { data } = await axios.get(
      API_URL + "/album/info/" + context.params.id,
      {
        headers: {
    ...
    const tracks = await axios.get(
      API_URL + "/album/tracks/" + context.params.id,
      {
        headers: {
          authorization: "Bearer " + token,
        },
