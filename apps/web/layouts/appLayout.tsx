...
function AppLayout({ children, title, color, onScroll }: any) {
  ...
  const [hydrated, setHydrated] = useState(false); // Estado de hidratación

  // Iniciar sesión automáticamente si es necesario
  useLogin();

  // Verificar si el componente está hidratado
  useEffect(() => {
    setHydrated(true);
  }, []);

  // Renderizar un indicador de carga hasta que el cliente esté hidratado
  if (!hydrated) {
    return null;
  }
 ...
 <Image
  src="/logo.png"
  alt="logo"
  layout="fill" // Asegura que la imagen ocupe el espacio completo del contenedor.
  objectFit="contain" // Ajusta cómo se escala la imagen dentro del contenedor.
  unoptimized={true}
/>
                </div>
                <h1
  className="text-center mx-2 text-md tracking-wider font-ProximaBold mini-laptop:hidden"
>
  super
  <span
    className="text-[#009bdd] font-bold text-lg tracking-wide"
  >
    audio 
  </span>
</h1>
